...
系统以日期为逻辑关联维度，但不为每一天预建空记录。`calendar_days` 只保存用户显式设置或未来导入的日期分类与标记；没有记录的日期按星期推断。`habit_logs`、`tasks`、`daily_health`、`journals` 和解析后的日期属性通过 ISO 生活日期在服务层与 `/api/day/{date}` 聚合。
...
首次启动创建表、索引并写入 `schema_version = 1`，随后执行 `PRAGMA integrity_check`。已有业务表但缺少 `schema_meta`，或 schema 版本不兼容时，程序必须停止写入。每个 SQLite 连接启用 `foreign_keys = ON`、`busy_timeout = 5000`、WAL 和 `synchronous = NORMAL`；正常关闭时执行 WAL checkpoint。开发阶段任何模型变更都必须同步更新本文、自动化测试和 [[接口验收清单]]。进入有真实用户数据的版本后，禁止用删除数据库再初始化代替迁移；需要采用可回滚或可备份恢复的版本化迁移流程。
