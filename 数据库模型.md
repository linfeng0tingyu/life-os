...
界面主题是可丢弃的表现层偏好，不属于领域数据。v0.1 不新增 `themes`、`user_preferences` 或类似数据库表，也不修改 schema v2；选择值仅存于桌面 WebView 的 `LIFE_OS_HOME/cache/webview/`。清空主题偏好不得影响任何任务、习惯、健康、日记、日历或财务记录。

M5 任务与习惯闭环复用既有 `tasks`、`habits` 和 `habit_logs`，schema 继续保持 v2。任务恢复只清空 `tasks.archived_at`；习惯停用与恢复只切换 `habits.active`；每日完成切换按 `(habit_id, date)` upsert，并保留既有数值、单位和备注。界面排序只更新现有 `sort_order`，不建立额外排序表。
...
`date UK` 在 `habit_logs` 中表示与 `habit_id` 组成联合唯一键，而不是单独唯一。
...
首次启动创建表、索引并写入 `schema_version = 2`，随后执行 `PRAGMA integrity_check`。已有 schema v1 在启动时只新增财务表并原地升级为 v2，不修改或删除既有生活数据。已有业务表但缺少 `schema_meta`，或 schema 版本高于程序支持范围时，程序停止写入。每个 SQLite 连接启用 `foreign_keys = ON`、`busy_timeout = 5000`、WAL 和 `synchronous = NORMAL`；正常关闭时执行 WAL checkpoint。后续非纯新增表的结构变更应引入完整的版本化迁移工具。
