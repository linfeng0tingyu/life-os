...
SQLite 数据库是业务数据的权威副本，保存于 `LIFE_OS_HOME/database/life.db`。配置、日志、备份、导出和缓存属于同一运行时根目录，但不因此写入业务表。v0.1 不建立用户、权限、AI、统计快照或文件附件表。
...
M5 任务与习惯闭环使用 `tasks`、`habits` 和 `habit_logs`；2026-09-21 的分类增量将 schema 升级为 v3，并新增 `categories` 作为任务与习惯可创建单选分类的权威选项表。任务恢复只清空 `tasks.archived_at`；习惯停用与恢复只切换 `habits.active`；每日完成切换按 `(habit_id, date)` upsert，并保留既有数值、单位和备注。界面排序只更新现有 `sort_order`，不建立额外排序表。

系统以日期为逻辑关联维度，但不为每一天预建空记录。`calendar_days` 只保存用户显式设置或未来导入的日期分类与标记；没有记录的日期按星期推断。`habit_logs`、`tasks`、`daily_health`、`journals`、`finance_transactions` 和解析后的日期属性通过 ISO 生活日期在服务层与 `/api/day/{date}` 聚合。
...
`date UK` 在 `habit_logs` 中表示与 `habit_id` 组成联合唯一键，而不是单独唯一。`categories` 与任务/习惯之间是由 `scope + name` 约束的逻辑关系。为兼容 schema v1/v2 已有文本数据，`tasks.category` 与 `habits.category` 暂不改为外键；服务层只允许写入相应作用域中已存在的规范名称。
...
`day_type` 是工作/休息语义的唯一依据，`holiday_name` 和 `custom_label` 不隐式改变日期类型。相同日期重复设置执行 upsert 并复用原记录；已归档记录再次设置时恢复该记录。索引 `(archived_at, date)` 支持有效覆盖查询，`date` 唯一约束防止同日多条冲突记录。
...
|---|---|---|---|
| `id` | INTEGER | PK | 分类标识 |
| `scope` | TEXT | NOT NULL, CHECK | `task` 或 `habit` |
| `name` | TEXT | NOT NULL, NOCASE | 显示名称；同一作用域内不区分大小写唯一 |
| `sort_order` | INTEGER | NOT NULL, DEFAULT 0 | 选项排序 |
| `created_at` | TEXT | NOT NULL | 创建时间 |
| `updated_at` | TEXT | NOT NULL | 更新时间 |

唯一约束为 `(scope, name)`，索引 `(scope, sort_order, id)` 支持稳定列出。任务与习惯可使用同名分类，但不能跨作用域误用；空分类继续用 `NULL` 表示“未分类”。v0.1 只开放创建与列出，不提供删除或重命名，避免已有记录失去选项来源。
...
- 日期显式分类按 `date` upsert；清除操作写入 `archived_at`，解析结果恢复为星期推断。
