...
系统以日期为逻辑关联维度，但不创建空的 `days` 表。`habit_logs`、`tasks`、`daily_health` 和 `journals` 通过 ISO 生活日期在服务层与 `/api/day/{date}` 聚合，避免仅为关联而创建没有业务内容的每日记录。
...
- 生活日期使用 `TEXT`，格式固定为 `YYYY-MM-DD`，由服务层做严格日历校验。
...
首次启动在单个事务中创建表、索引和初始 schema 版本。开发阶段任何模型变更都必须同步更新本文、自动化测试和 [[接口验收清单]]。进入有真实用户数据的版本后，禁止用删除数据库再初始化代替迁移；需要采用可回滚或可备份恢复的版本化迁移流程。
