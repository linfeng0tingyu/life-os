...
M5 任务与习惯闭环使用 `tasks`、`habits` 和 `habit_logs`；2026-09-21 的分类增量在 schema v3 新增 `categories`，M6 后续把 schema 升到 v4并扩展 `finance` 作用域，使任务、习惯和财务均可使用独立的可创建单选分类。信用卡账期增量把 schema 升到 v5，在 `finance_accounts` 增加可迁移的账单日字段，不新增账单快照表。任务恢复只清空 `tasks.archived_at`；习惯停用与恢复只切换 `habits.active`；每日完成切换按 `(habit_id, date)` upsert，并保留既有数值、单位和备注。界面排序只更新现有 `sort_order`，不建立额外排序表。
...
账户当前余额由期初余额与未归档流水动态计算，不单独保存可与流水分离的缓存余额。资产账户期初余额不得为负，负债账户用负数表示待偿还余额。`credit` 必须使用负债性质；新建信用卡未提供 `billing_day` 时使用 18，旧信用账户迁移到 schema v5 时同样补为 18。账期、上期账单、已还金额和待还金额均由账单日与流水实时派生，不保存可能与流水失去一致性的账单缓存。
...
首次启动创建表、索引并写入 `schema_version = 5`，随后执行 `PRAGMA integrity_check`。已有 schema v1 先补齐财务表，schema v1/v2 再新增 `categories`；schema v3 会安全重建分类约束以加入 `finance` 作用域；schema v4 为财务账户增加 `billing_day`，并为已有 `credit` 账户补入默认值 18。分类迁移会把 `tasks.category`、`habits.category` 和 `finance_transactions.category` 中既有的非空名称按作用域回填为分类选项，不修改或删除原业务记录。已有业务表但缺少 `schema_meta`，或 schema 版本高于程序支持范围时，程序停止写入。每个 SQLite 连接启用 `foreign_keys = ON`、`busy_timeout = 5000`、WAL 和 `synchronous = NORMAL`；正常关闭时执行 WAL checkpoint。后续非纯新增表的结构变更应引入完整的版本化迁移工具。
