...
M5 已完成 `/tasks` 与 `/habits` 独立管理页。任务支持创建、编辑、开始、完成、取消、重新打开、排序、归档与恢复；习惯支持创建、编辑、排序、停用、恢复，以及今天或过去日期的即时打卡。Today 与 Calendar 的日期详情可直接切换任务和习惯状态，写入期间会阻止重复触发。健康、日记和财务的完整交互仍在 M6。
