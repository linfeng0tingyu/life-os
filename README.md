...
M4 已完成 Today Dashboard、独立日历页、任意日期访问、未来任务快速规划、工作/休息日标记、统一 API Client 和前端状态框架。“今日”只呈现设备本地当天数据；过去与未来月份、完整日期聚合和节假日/自定义标记统一在 `/calendar` 中处理。日历详情与 Today 使用相同的任务、习惯、生活节律、日记和财务布局，其中生活节律概览只显示睡眠时长、体重、运动和身体健康；未来日期可以新增安排到当天的任务。主内容随视口自适应填满可用空间，超宽屏自动分栏、窄屏保持单列。任务与习惯的完整交互在 M5；健康、日记和财务的完整交互在 M6。
